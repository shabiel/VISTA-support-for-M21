...
LISTEN(SOCK,RTN,ZRULE) ;Listen on socket, start routine
 ...
 S POP=1 D LONT:ZISOS["OpenM",LGTM:ZISOS["GT.M",LM21:ZISOS["M21"
...
LM21 ;
 ...
 n dev s dev=9050
 s (POP)=0
 D LOG("Open for Listen M21")
 o dev:(::SOCK):timeout
 i $dev D LOG("couldn't open...") quit  ; w !,”Unable to create server socket - $device=”,$dev c dev q
 ; Establish a listen queue depth of 3
 u dev w /listen(3)
 d LOG("Listening...")
 ; Wait for a client connection
 f  w /wait(timeout) q:$dev  S POP=$$EXIT Q:POP  i $key'="" d
 . d LOG("Got connection...")
 . S IO=dev
 . ; U IO:(:::$c(13,10))
 . U IO
 . D VAR,@RTN
 ...
 . u dev:(::::::handle)
 . ; j CHILDM21^%ZISTCPS(handle,RTN)
 . c dev:handle
 . S POP=$$EXIT
 close dev
 quit
 ;
CHILDM21(IO,RTN) ; Child for M21
 S $ETRAP="D ^%ZTER L  HALT"
 n dev s dev=9050
 o dev:(:::::IO)
 U IO:(:::$c(13,10))
 I $G(^%ZIS(14.5,"LOGON",^%ZOSF("VOL"))) D  QUIT
 . W "421 Service temporarily down.",$C(13,10),!
 D VAR,@RTN
 HALT
 ;
